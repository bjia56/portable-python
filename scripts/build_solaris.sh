...
function build_headless_deps () {
  ...
  cd ncurses*
  maybe_patch
  ./configure --with-normal --without-shared --without-progs --without-cxx-binding --enable-overwrite --disable-stripping --enable-widec --with-termlib --disable-database --with-fallbacks=xterm,xterm-256color,screen-256color,linux,vt100 --with-tic-path=/usr/bin/gtic --with-infocmp-path=/usr/bin/ginfocmp --prefix=${DEPSDIR}
  gmake -j4
  gmake install.libs
  install_license
  ...
  cd gdbm*
  maybe_patch
  CFLAGS="${CFLAGS} -std=gnu17" ./configure --enable-libgdbm-compat --disable-shared --prefix=${DEPSDIR}
  gmake -j4
  gmake install
  install_license
