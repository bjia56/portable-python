...
function build_headless_deps () {
  ...
  cd ncurses*
  maybe_patch
  ./configure --with-normal --without-shared --without-progs --without-cxx-binding --enable-overwrite --disable-stripping --enable-widec --with-termlib --disable-database --with-fallbacks=xterm,xterm-256color,screen-256color,linux,vt100 --prefix=${DEPSDIR}
  gmake -j4
  gmake install.libs
  install_license
