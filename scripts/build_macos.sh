...
cd ncurses*
CC=clang CXX=clang++ CFLAGS="${CFLAGS} -arch x86_64 -arch arm64" CXXFLAGS="${CXXFLAGS} -arch x86_64 -arch arm64" ./configure --with-normal --without-progs --without-termlib --without-ticlib --enable-overwrite --disable-stripping --prefix=${DEPSDIR}
make -j4
make install
install_license
